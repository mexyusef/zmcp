...
class MainWindow(QMainWindow):
    ...
    def _connect_signals(self):
        ...
        self.client_panel.tool_selected.connect(self._on_client_tool_selected)

        # Tools panel signals
        self.tools_panel.tool_selected.connect(self._on_tools_panel_tool_selected)
    ...
    def _show_about(self):
        ...
        QMessageBox.about(
            ...
            <p>Version 1.0</p>
            <p>A human-oriented, high-performance PyQt6 application
            implementing the Model Context Protocol (MCP).</p>
            <p>Provides both server and client capabilities in a modern,
            modular interface designed to deliver exceptional developer
    ...
    def _on_server_started(self):
        ...
        # Update tools panel with server tools
        if hasattr(self.server_panel, "mcp_server") and self.server_panel.mcp_server:
            server_url = f"http://{self.server_panel.server_config.get('host', 'localhost')}:{self.server_panel.server_config.get('port', 8000)}"
            server_name = self.server_panel.server_config.get("name", "Local Server")

            # Update tools panel
            self.tools_panel.update_server_tools(
                server_url,
                server_name,
                self.server_panel.mcp_server.tools.values()
            )

            self.tools_panel.update_server_resources(
                server_url,
                server_name,
                self.server_panel.mcp_server.resources.values()
            )

            self.tools_panel.update_server_prompts(
                server_url,
                server_name,
                self.server_panel.mcp_server.prompts.values()
            )
    ...
    def _on_server_stopped(self):
        ...
        if hasattr(self.server_panel, "server_config"):
            server_url = f"http://{self.server_panel.server_config.get('host', 'localhost')}:{self.server_panel.server_config.get('port', 8000)}"
            self.tools_panel.remove_server(server_url)

    def _on_client_connected(self, url):
        """Handle client connected event."""
        self.status_bar.showMessage(f"Connected to {url}")

        # Update tools panel with client tools
        if hasattr(self.client_panel, "client") and self.client_panel.client:
            # Update tools panel
            self.tools_panel.update_server_tools(
                url,
                "Remote Server",
                self.client_panel.client.tools
            )

            self.tools_panel.update_server_resources(
                url,
                "Remote Server",
                self.client_panel.client.resources
            )

            self.tools_panel.update_server_prompts(
                url,
                "Remote Server",
                self.client_panel.client.prompts
            )
    ...
    def _on_client_tool_selected(self, tool_name, server_url):
        """Handle tool selection from client panel."""
        self.status_bar.showMessage(f"Selected tool: {tool_name}")

    def _on_tools_panel_tool_selected(self, tool_name, server_url):
        """Handle tool selection from tools panel."""
        self.status_bar.showMessage(f"Selected tool: {tool_name} from {server_url}")

        # Check if this is a local server
        if hasattr(self.server_panel, "server_config"):
            local_server_url = f"http://{self.server_panel.server_config.get('host', 'localhost')}:{self.server_panel.server_config.get('port', 8000)}"
            if server_url == local_server_url:
                # If server is running, we can execute the tool directly
                if hasattr(self.server_panel, "server_running") and self.server_panel.server_running:
                    self.tabs.setCurrentWidget(self.server_panel)
                    return

        # Otherwise, check if we need to connect to a remote server
        if hasattr(self.client_panel, "server_url") and self.client_panel.server_url == server_url:
            # Already connected to this server, just select the tool
            self.tabs.setCurrentWidget(self.client_panel)

            # Find the tool in the client's tools table and select it
            for row in range(self.client_panel.tools_table.rowCount()):
                if self.client_panel.tools_table.item(row, 0).text() == tool_name:
                    self.client_panel.tools_table.selectRow(row)
                    self.client_panel._tool_clicked(self.client_panel.tools_table.item(row, 0))
                    break
        else:
            # Need to connect to the server first
            self.client_panel.url_edit.setText(server_url)
            self.tabs.setCurrentWidget(self.client_panel)
            self.client_panel._connect_to_server()
